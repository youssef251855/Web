...
import { autoSeedTableRows } from "@/lib/table-generator";
...
export default function BuilderPage() {
  ...
  const [isBuilderAppwriteModalOpen, setIsBuilderAppwriteModalOpen] = useState(false);
  ...
  const applyWhatsAppTemplate = async () => {
    ...
    try {
      ...
      let contactTable = userTables?.find((t: any) => t.name === "جهات اتصال واتساب");
      let messageTable = userTables?.find((t: any) => t.name === "رسائل واتساب Web");
      ...
      setTimeout(async () => {
        ...
        alert("🎉 تم إعداد قاعدة البيانات جهات الاتصال والرسائل في Appwrite ومزامنتها بنجاح مع 3 صفحات تفاعلية! يمكنك التحقق منها بالمعاينة الآن.");
      }, 500);
  ...
  const handleCreateProjectTable = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-generate 3 sample rows immediately for live data preview
      ...
      if (seedResult.success && seedResult.inserted) {
        setDbEditRecords(seedResult.inserted);
      }

      alert("🎉 تم إنشاء الجدول وتوليد 3 صفوف تجريبية أولية تلقائياً بنجاح!");
    } catch (error: any) {
  ...
  const [isDbEditGeneratingRows, setIsDbEditGeneratingRows] = useState(false);

  const handleAutoGenerateBuilderRows = async (count: number = 3) => {
    if (!dbEditSelectedTable || !user) return;
    setIsDbEditGeneratingRows(true);
    try {
      const seedResult = await autoSeedTableRows(
        dbEditSelectedTable.id,
        user.id,
        dbEditSelectedTable.name,
        dbEditSelectedTable.fields,
        count
      );
      if (seedResult.success && seedResult.inserted) {
        setDbEditRecords(prev => [...prev, ...seedResult.inserted]);
      } else {
        alert("تعذر توليد الصفوف تلقائياً. تأكد من إعدادات قاعدة البيانات.");
      }
    } catch (err) {
      console.error("Error generating builder table rows:", err);
    } finally {
      setIsDbEditGeneratingRows(false);
    }
  };

  const handleExportBuilderCSV = () => {
    if (!dbEditSelectedTable) return;
    const success = exportRecordsToCSV(dbEditSelectedTable.name, dbEditSelectedTable.fields, dbEditRecords);
    if (!success) {
      alert('حدث خطأ أثناء تصدير ملف CSV');
    }
  };

  const handleDownloadBuilderTemplate = () => {
    if (!dbEditSelectedTable) return;
    downloadSampleCSVTemplate(dbEditSelectedTable.name, dbEditSelectedTable.fields);
  };

  const handleImportBuilderCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!dbEditSelectedTable || !user) return;
    ...
    reader.onload = async (event) => {
      try {
        ...
        const existingFieldNames = new Set((dbEditSelectedTable.fields || []).map((f: any) => f.name.toLowerCase()));
        ...
        let currentFields = dbEditSelectedTable.fields || [];
        if (newFieldsToAdd.length > 0) {
          currentFields = [...currentFields, ...newFieldsToAdd];
          await supabase
            .from('tables')
            .update({ fields: JSON.stringify(currentFields) })
            .eq('id', dbEditSelectedTable.id);
          
          const updatedTable = { ...dbEditSelectedTable, fields: currentFields };
        ...
        setDbEditRecords(prev => [...prev, ...insertedRecords]);
        alert(`🎉 تم استيراد ${insertedRecords.length} صف بنجاح من ملف الـ CSV!`);
      } catch (error: any) {
        console.error('Failed to import CSV into builder table:', error);
          ...
          <div className="space-y-4">
            ...
            <div className="text-xs text-blue-600 bg-blue-50 border border-blue-200 p-2.5 rounded-md text-left">
              ...
              <ul className="list-disc pl-4 mt-1 space-y-1">
                ...
                <li>عندما يقوم الزوار بزيارة موقعك ورفع ملف، سيتم تخزين الملف في Appwrite Storage وحفظ رابطه في العمود المختار من الجدول!</li>
              </ul>
            </div>
          </div>
        );
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 overflow-hidden">
      ...
      {topTab === "editor" && (
        <>
          <div className="flex flex-1 overflow-hidden relative">
            ...
            {/* Canvas */}
            <main
              className={`${mobileView === "canvas" ? "flex" : "hidden"} md:flex flex-col flex-1 relative overflow-y-auto overflow-x-hidden bg-white pb-20 md:pb-0`}
              onClick={(e) => {
            ...
            >
              ...
              <div
                ...
                className="w-full min-h-[1400px] md:min-h-screen bg-white relative pb-32 md:pb-12"
              >
                <BuilderCanvasMap
                  canvasRef={canvasRef}
                  setMobileView={setMobileView}
            ...
            {dbEditSelectedTable ? (
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {dbEditViewMode === 'data' ? (
                  <div className="flex-1 flex flex-col overflow-hidden">
                    {/* Data Toolbar */}
                    <div className="px-6 py-3 border-b bg-gray-50 flex items-center justify-between shrink-0 flex-wrap gap-2">
                      ...
                      <div className="flex items-center space-x-2 space-x-reverse flex-wrap gap-1">
                        <div className="flex items-center rounded-md border border-indigo-200 bg-indigo-50/70 p-0.5 shadow-2xs">
                          <button
                            onClick={() => handleAutoGenerateBuilderRows(3)}
                            disabled={isDbEditGeneratingRows}
                            className="text-indigo-700 hover:bg-indigo-100/80 px-2.5 py-1 rounded-sm transition flex items-center text-3xs font-semibold disabled:opacity-50 cursor-pointer"
                            title="إنشاء 3 صفوف ذكية تلقائياً مع قيم نموذجية مطابقة لنوع الحقول"
                          >
                            <Sparkles className="w-3 h-3 ml-1 text-indigo-600 animate-pulse" />
                            {isDbEditGeneratingRows ? 'جاري التوليد...' : 'توليد صفوف تلقائياً (3)'}
                          </button>
                          <div className="h-3.5 w-px bg-indigo-200 mx-0.5" />
                          <button
                            onClick={() => handleAutoGenerateBuilderRows(5)}
                            disabled={isDbEditGeneratingRows}
                            className="text-indigo-600 hover:bg-indigo-100/80 px-1.5 py-1 rounded-sm transition text-3xs font-medium disabled:opacity-50 cursor-pointer"
                            title="توليد 5 صفوف"
                          >
                            +5
                          </button>
                        ...
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-md shadow-2xs flex items-center transition cursor-pointer"
                          title="تصدير بيانات الجدول إلى ملف CSV"
                        >
                        ...
                        <input
                          type="file"
                          accept=".csv,text/csv"
                          id={`builder-import-csv-${dbEditSelectedTable.id}`}
                          className="hidden"
                          onChange={handleImportBuilderCSV}
                        />
                        <button
                          onClick={() => document.getElementById(`builder-import-csv-${dbEditSelectedTable.id}`)?.click()}
                          className="px-2.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-300 font-semibold text-xs rounded-md shadow-2xs flex items-center transition cursor-pointer"
                          title="استيراد صفوف من ملف CSV"
                        >
                          <FileSpreadsheet className="w-3.5 h-3.5 ml-1" /> استيراد CSV
                        </button>

                        <button
                          onClick={handleDownloadBuilderTemplate}
                          className="px-2 py-1.5 text-gray-600 hover:bg-gray-100 border border-gray-200 font-medium text-xs rounded-md shadow-2xs transition cursor-pointer"
                          title="تحميل نموذج CSV فارغ"
                        >
                          نموذج CSV
                        </button>
                        ...
                        <button
                          onClick={() => setIsBuilderAppwriteModalOpen(true)}
                          className="px-2.5 py-1.5 bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white font-semibold text-xs rounded-md shadow-2xs flex items-center transition cursor-pointer gap-1"
                          title="تصدير وتنزيل ملفات وقوالب Appwrite CSV"
                        >
                          <Database className="w-3.5 h-3.5" /> Appwrite CSV
                        </button>
                        ...
                        >
                          <Plus className="w-3.5 h-3.5 ml-1" /> إضافة سجل جديد
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-auto">
                      {dbEditLoadingRecords ? (
                        ...
                        <div className="p-16 text-center flex flex-col items-center justify-center max-w-sm mx-auto">
                          ...
                          <p className="text-gray-700 text-xs font-bold mb-1">لا يوجد بيانات مسجلة في هذا الجدول حالياً.</p>
                          <p className="text-3xs text-gray-400 mb-4 text-center">يمكنك توليد صفوف تجريبية تلقائية لتغذية المكونات والقوائم بالبيانات فوراً أو إضافة سجل يدوي.</p>
                          <button
                            onClick={() => handleAutoGenerateBuilderRows(3)}
                            disabled={isDbEditGeneratingRows}
                          ...
                          >
                            ...
                            {isDbEditGeneratingRows ? 'جاري التوليد التلقائي...' : '✨ إنشاء 3 صفوف تلقائية الآن'}
                          </button>
                        </div>
                      ) : (
                                ...
                                {dbEditRecords.map((record) => (
                                  <tr key={record.id} className="hover:bg-gray-50/50 transition">
                                    ...
                                    {dbEditSelectedTable.fields.map((field: any, index: number) => {
                                      ...
                                      const isImage = isUrl && (val.match(/\.(jpeg|jpg|gif|png|webp|svg|bmp)/i) || val.includes('appwrite') || val.includes('supabase') || val.includes('cloudinary'));
      ...
      {/* Appwrite CSV Modal */}
      <AppwriteCSVModal
        isOpen={isBuilderAppwriteModalOpen}
        onClose={() => setIsBuilderAppwriteModalOpen(false)}
        currentTable={dbEditSelectedTable ? { name: dbEditSelectedTable.name, fields: dbEditSelectedTable.fields } : undefined}
        currentRecords={dbEditRecords}
      />
    </div>
  );
}
