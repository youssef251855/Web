...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (fetchedUserId) {
      let { data: pageSnap } = await supabase
        .from('pages')
      ...
      if (pageSnap && pageSnap.length > 0) {
        ...
        const displayTitle = pageData.title || `${username} - Portfolio`;
        const displayDescription = pageData.description || `${username}'s custom page built with Joex`;
        return {
          title: displayTitle,
  ...
  const defaultTitle = `${username} - Portfolio`;
  const defaultDesc = `${username}'s custom page built with Joex`;
  return {
    title: defaultTitle,
